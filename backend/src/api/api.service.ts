...
export class ApiService {
  ...
  async updateAdminTicket(ticketId: number, data: { status?: string; priority?: string; admin_id?: number }) {
    return this.prisma.support_tickets.update({
      where: { ticket_id: ticketId },
  ...
  async getAdminReportStats(period: string = '30d') {
    const now = new Date();
    ...
    const [
      ...
      totalRevenue, totalUsers, totalTaskers,
      recentOrders, topServices, topTaskers,
      newUsersThisPeriod, ordersByDay,
    ] = await Promise.all([
    ...
    ordersByDay.forEach(o => {
      ...
      dailyMap[day].orders++;
      if (o.status === 'COMPLETED') dailyMap[day].revenue += Number(o.total_price || 0);
    });
